...
function LinkUserTransporter() {
  ...
  const handleDeleteClick = async (params) => {
    ...
    let deleteResponce = await AdminServices.deleteUserTransporterLink(
      rowValues.id
    );
    ...
    if (deleteResponce.code === 200) {
      ...
      setSuccessMessage(
        "User " +
          rowValues.user +
          " To Vendor " +
          rowValues.transporterName +
          " Link Deleted"
      );

      setOpen(true);
      getUserTransporterList();
    } else {
  ...
  const [plantIdForT, setPlantIdForT] = useState();
  ...
  async function getTrasnporter(id) {
  ...
  async function getPlants() {
    ...
    if (plantList.code === 200) {
      ...
      setDialogTitle("Getting Plant list Error");
      setSuccessMessage("Something went wrong, check your network!!");
      setOpen(true);
    }
  }

  async function getUserTransporterList() {
    let userTransporterList = await AdminServices.getUserTransporterList();

    if (userTransporterList.code === 200) {
      setRows([]); //Clear existing list in array
      userTransporterList.responseBody.map((data, index) =>
        setRows((prevState) => [
          ...prevState,
          {
            id: data.id,
            ...
            transporterName:
              data.transporter.name + " (" + data.transporter.code + ")",
            plant: data.company.name,
          },
        ])
      );
    } else {
  ...
  const add = async (e) => {
    if (!transporterId) {
      ...
      setSuccessMessage("Select a transporter to link Vendor");
      setOpen(true);
      return;
    }
    ...
    if (!userId) {
      ...
      setSuccessMessage("Select a User to link transporter");
      setOpen(true);
      return;
    }
    ...
    let linkUserTransporterResponce = await AdminServices.linkUserToTransporter(
      data
    );

    if (linkUserTransporterResponce.code === 200) {
      setDialogTitle("User Linked to Transporter Successfully");
      setSuccessMessage("Success");
      setOpen(true);
      getUserTransporterList();
    } else {
      if (linkUserTransporterResponce.code === 209) {
        setDialogTitle("Failed To Link");
        setSuccessMessage(linkUserTransporterResponce.responseBody);
        setOpen(true);
      }
    }
  };
...
export default LinkUserTransporter;

...
function CorporateMaster() {
  ...
  async function getRoleList() {
    let response = await AdminServicesObj.getCompanyByContext("corporate");
    if (response.code === 200) {
      setCorporateList(
        response.responseBody.map((corporate) => ({
  ...
  async function getPlantList() {
    let plantList = await AdminServicesObj.getCompanyByContext("plant");

    if (plantList.code === 200) {
      setAllCorporate([]);
      plantList.responseBody.map((vendor, index) =>
  ...
  const vendorMasterAdd = async (e) => {
    if (corporateId.length === 0) {
      setDialogTitleMessage("Corporate is empty");
      setDialogContentMessage("Please select a Corporate");
      setOpen(true);
      return;
    }
    if (code.length === 0) {
      setDialogTitleMessage("Plant Code is empty");
      setDialogContentMessage("Enter Valid vendor code");
      setOpen(true);
      return;
    }
    if (name.length === 0) {
      setDialogTitleMessage("Plant Name is empty");
  ...
  return (
    <Grid container style={containerStyle}>
      <h3 style={{ marginLeft: "8px" }}>Add Plant</h3>
          ...
          <ThemeProvider theme={formLabelsTheme}>
            <form noValidate autoComplete="off">
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6} md={4} lg={2}>
                  <Autocomplete
                    {...corporateListDropdown}
                    disablePortal
                    id="CorporateDropDown"
                    sx={{ width: "90%", marginLeft: "10px", marginTop: "8px" }}
                    onChange={(e, r) => {
                      if (r != null) {
                        setCorporateName(r.label);
                        setCorporateId(r.id);
                      }
                    }}
                    renderInput={(params) => (

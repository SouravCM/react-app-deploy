...
function VehicleActivity() {
  ...
  const [vehicleActivityList, setVehicleActivity] = useState([]);
  ...
  const getVehicleActivityDetails = async (e) => {
    setLoading(true);
    ...
    let vehicleActivityData = await transporterService.vehicleActivity(data);

    setVehicleActivity([]);
    if (vehicleActivityData.code === 200) {
      vehicleActivityData.responseBody.map((vehicleActivity, index) =>
        setVehicleActivity((prevState) => [
          ...prevState,
          {
            id: index + 1,
            sno: index + 1,
            date: moment(new Date(vehicleActivity.date)).format(
              "DD-MM-YYYY HH:mm:ss"
            ),
            runTime: vehicleActivity.runTimeInHHMM,
            idealTime: vehicleActivity.idleTimeInHHMM,
            stoppageTime: vehicleActivity.stopTimeInHHMM,
          },
        ])
      );
      setLoading(false);
    } else if (vehicleActivityData.code === 208) {
  ...
  const columns = [
    ...
    {
      field: "stoppageTime",
      headerName: "Stoppage Time(HH:MM)",
      align: "center",
      headerAlign: "center",
      width: "auto",
      flex: 1,
  ...
  return (
    <div>
      ...
      <Grid container style={containerStyle}>
        <h3 style={{ marginLeft: "8px" }}>Vehicle Activity</h3>

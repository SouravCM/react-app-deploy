...
function DebitNoteReport() {
  ...
  const getDnDetails = async (e) => {
    ...
    let dnResponceData = await transporterService.getDNList(data);
    console.log("Invoice:" + JSON.stringify(dnResponceData));
    setDnList([]);
    if (dnResponceData.code === 200) {
      dnResponceData.responseBody.map((dn, index) =>
        setDnList((prevState) => [
          ...prevState,
          {
            id: dn.id,
            ...
            level2Status: dn.level2Status,
            level2ApprovedBy: dn.level2Approver ? dn.level2Approver.name : "",
            level2ApprovedOn: dn.level2ApprovedOn,
            level2Remarks: dn.level2Remarks,
          },
        ])
      );
    } else if (dnResponceData.code === 208) {

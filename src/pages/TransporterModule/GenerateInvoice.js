  ...
  return (
    <GridToolbarContainer>
      ...
      <Button
        color="success"
        startIcon={<SaveIcon />}
        onClick={props.handleGenerateIncoive}
      >
        Generate Invoice
      </Button>
    </GridToolbarContainer>
...
function GenerateInvoice(props) {
  ...
  const getformInvoice = async (e) => {
    ...
    console.log("id", data);
    let invoiceData = await transporterService.getFormInvoice(data);
    console.log("Responce" + JSON.stringify(invoiceData));
    if (invoiceData.code === 200) {
      ...
      setDialogContentMessage(invoiceData.responseBody);
      setOpen(true);
    }
  };
  ...
  const handleGenerateIncoive = async () => {
    setLoading(true);
    if (gst.length === 0) {
    ...
    let invoiceSavedData = await transporterService.addInvoice(invoiceSaveData);
    console.log("Responce" + JSON.stringify(invoiceSavedData));
    if (invoiceSavedData.code === 209) {
      setLoading(false);
      setDialogTitleMessage("Invoice Not Generated Created");
      setDialogContentMessage(invoiceSavedData.responseBody);
      setOpen(true);
    } else if (invoiceSavedData.code === 200) {
      setLoading(false);
      setDialogTitleMessage("Invoice Generated Successfully");
      setDialogContentMessage("");
      setOpen(true);
      setComplited(true);
    } else {
              ...
              slots={{
                toolbar: () => (
                  <EditToolbar
                    ...
                    handleGenerateIncoive={handleGenerateIncoive}
                    getformInvoice={getformInvoice}
                  />
                ),
              }}

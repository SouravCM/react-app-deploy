...
const InvoiceLevel1Approval = () => {
  ...
  const handleApprove = async () => {
    ...
    const jsonBody = {
      ...
      level1ApprovedOn: formattedDate,
    };
    // console.log("data- " + JSON.stringify(jsonBody));

    const responseApproval = await transporterService.InvoiceUpdateLevel1Status(
      jsonBody
    );
  ...
  const handleReject = async () => {
    ...
    if (remarks.length === 0) {
      ...
      const responseApproval =
        await transporterService.InvoiceUpdateLevel1Status(jsonBody);
  ...
  async function getInvoiceLevel1() {
    ...
    let requestBody = { companyId: plantId, level1Status: "PENDING" };
...
export default InvoiceLevel1Approval;

...
function GenerateDebitNote() {
  ...
  async function getInvoice() {
    ...
    let InvoiceList = await transporterService.getInvoiceBasicInfo(data);
    ...
    if (InvoiceList.code === 200) {
      ...
      setDialogTitleMessage("Getting Invoice list Error");
      setDialogContentMessage("Something went wrong, check your network!!");
      setOpen(true);
    }
  }

  async function fromDebitNote(id) {
    let debitNoteDataResponce = await transporterService.formDebitNote(id);
    ...
    setTransporterId(debitNoteDataResponce.responseBody.transporter.id);
    setRows([]);
  ...
  const handleGenerateDebit = async () => {
    ...
    if (debitSavedData.code === 209) {
      setLoading(false);
      setDialogTitleMessage("Debit Not Generated Created");
      setDialogContentMessage(debitSavedData.responseBody);
      setOpen(true);
    } else if (debitSavedData.code === 200) {
      setLoading(false);
      setDialogTitleMessage("Debit Note Generated Successfully");
      ...
      setDialogContentMessage("unable to generate invoice check your network");
      setOpen(true);
    }
  };
            ...
            <FormControl sx={formControlStyle}>
              ...
              >
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField {...params} label="Invoice Number" />
                  )}
                />
              </div>
            </FormControl>
        ...
        <div style={{ width: "100%" }}>
          ...
          >
            <DataGrid
              ...
              slots={{
                toolbar: () => (
                  <EditToolbar handleGenerateDebit={handleGenerateDebit} />
                ),
              }}

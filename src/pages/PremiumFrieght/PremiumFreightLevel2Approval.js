...
const PremiumFreightLevel2Approval = () => {
  ...
  const handleApprove = async () => {
    ...
    const responseApproval = await FreightServices.approveRejectFrieghtLevel2(
      jsonBody
    );
  ...
  async function getFreightApprovalLevel2() {
    ...
    if (freightList.code === 200) {
      ...
      materialsArray.map((freight, index) => {
        if (freight.level1Status != "REJECTED") {
  ...
  return (
    <div>
      <Grid container style={containerStyle}>
        <h3 style={{ marginLeft: "8px" }}>Premium Freight Level 2 Approval</h3>
...
export default PremiumFreightLevel2Approval;
